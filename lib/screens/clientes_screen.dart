...
class _ClientesScreenState extends State<ClientesScreen> {
  ...
  void _onBuscarChanged(String value) {
    ...
    _debounce = Timer(const Duration(milliseconds: 350), () {
      _cargar(q: value.trim().isEmpty ? null : value.trim());
    });
  }
  ...
  Future<void> _anadir() async {
    ...
    try {
      ...
      rootScaffoldKey.currentState?.showSnackBar(
        SnackBar(content: Text(_errorToMessage(e, fallback: 'Error al crear el cliente.'))),
      );
    } finally {
      nombreCtrl.dispose();
      telefonoCtrl.dispose();
  ...
  Future<void> _editar(Cliente c) async {
    ...
    try {
      ...
      rootScaffoldKey.currentState?.showSnackBar(
        SnackBar(content: Text(_errorToMessage(e, fallback: 'Error al editar el cliente.'))),
      );
    } finally {
      nombreCtrl.dispose();
      telefonoCtrl.dispose();
  ...
  Future<void> _eliminar(Cliente c) async {
    ...
    try {
      ...
      rootScaffoldKey.currentState?.showSnackBar(
        SnackBar(content: Text(_errorToMessage(e, fallback: 'Error al eliminar el cliente.'))),
      );
    }
  }
                ...
                itemBuilder: (_, i) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text(c.telefono + (c.email != null ? '\n${c.email}' : '')),
              ...
              children: [
                Row(
                  children: [
                    ...
                    Text(widget.cliente.telefono, style: const TextStyle(fontSize: 14)),
                  ],
                ),
                ...
                if (widget.cliente.notas != null) ...[
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 6),
                      Expanded(child: Text(widget.cliente.notas!, style: const TextStyle(fontSize: 14))),
                    ],
                  ),
                ],
              ],
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 6),
                Text('Historial de viajes', style: TextStyle(color: Colors.grey.shade600, fontWeight: FontWeight.w600)),
                const Spacer(),
                Text('${_viajes.length} viajes', style: TextStyle(color: Colors.grey.shade500, fontSize: 13)),
              ],
            ),
          ),
          Expanded(
            child: _cargando
                ? const Center(child: CircularProgressIndicator())
                : _viajes.isEmpty
                ? const Center(child: Text('Sin viajes registrados.', style: TextStyle(color: Colors.grey)))
                : ListView.separated(
              padding: const EdgeInsets.all(12),
              ...
              itemBuilder: (_, i) {
                final v = _viajes[i];
                final fecha = DateFormat('dd/MM/yyyy').format(DateTime.parse(v.dia));
                return Card(
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                  child: ListTile(
                    leading: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(fecha, style: TextStyle(fontSize: 10, color: Colors.amber.shade700)),
                        ],
                      ),
                    ),
                    title: Text(
                    ...
                    subtitle: v.conductor != null
                        ? Text('${v.conductor!.nombre} · ${v.conductor!.matricula}', style: const TextStyle(fontSize: 12))
                        : null,
                  ),
                );
              },
